...
export function dimensionLines(layout,ids,{all=false,limit=30,hidden=false,unit='m',dec=2,size=12},camera,width,height){
 ...
 for(const d of drawingDimensions(layout).filter(d=>!hidden&&(d.id.startsWith('warehouse')||allowed.has(d.id)))){const length=Math.hypot(d.b.x-d.a.x,d.b.y-d.a.y);if(!length)continue;const nx=-(d.b.y-d.a.y)/length*d.offset,ny=(d.b.x-d.a.x)/length*d.offset,a={x:d.a.x+nx,y:d.a.y+ny},b={x:d.b.x+nx,y:d.b.y+ny};path([project(d.a),project(a),project(b),project(d.b)]);const mid=project({x:(a.x+b.x)/2,y:(a.y+b.y)/2});if(mid.visible){const t=document.createElementNS(NS,'text');t.setAttribute('x',mid.x);t.setAttribute('y',mid.y-5);t.setAttribute('text-anchor','middle');t.setAttribute('fill','var(--text)');t.setAttribute('font-size',Math.min(24,Math.max(9,size)));t.textContent=(length*(unit==='mm'?1000:1)).toFixed(dec)+unit;svg.append(t);}for(const [p,q]of [[a,b],[b,a]]){const A=project(p),B=project(q),ang=Math.atan2(B.y-A.y,B.x-A.x);path([{x:A.x+7*Math.cos(ang-.5),y:A.y+7*Math.sin(ang-.5),visible:A.visible},A,{x:A.x+7*Math.cos(ang+.5),y:A.y+7*Math.sin(ang+.5),visible:A.visible}]);}}
