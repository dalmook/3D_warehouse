...
/** Plan +Y points down the top view. CAD +Y points up: reflect the axis AND rotation. */
export const cadPoint=p=>point3d(p.x*CAD_SCALE,-p.y*CAD_SCALE,0);
...
export function exportDXF(layout){
 ...
 const blocks=new Map();for(const o of planObjects(layout)){const key=[o.type,o.width,o.depth].join(':');if(!blocks.has(key)){const name='EQ'+blocks.size,b=d.addBlock(name);polygon(b,footprint({...o,x:0,y:0,rotation:0}),'0');blocks.set(key,name);}d.addInsert(blocks.get(key),cadPoint(o),{rotationAngle:o.rotation,layerName:layer(o)});if(o.name)d.addText(cadPoint(o),120,o.config?.text||o.name,{layerName:'TEXT'});}
...
export async function reportPDF(layout,{paper='A3',scale=100,author='',approver='',revision='',blueprint=false}={}){
 ...
 const detail=document.createElement('canvas');detail.width=c.width;detail.height=c.height;const dc=detail.getContext('2d');dc.fillStyle='white';dc.fillRect(0,0,detail.width,detail.height);dc.fillStyle='#19374b';dc.font='bold 26px sans-serif';dc.fillText('설비 수량 / 팔레트 적재 상세',50,55);dc.font='18px sans-serif';let lineY=95;
 for(const [type,count]of quantities){dc.fillText((CATALOG[type]?.name||type)+' : '+count+'개',50,lineY);lineY+=26;}
 pages.push(detail);
 for(const o of loads){const page=document.createElement('canvas');page.width=c.width;page.height=c.height;const g=page.getContext('2d');g.fillStyle='white';g.fillRect(0,0,page.width,page.height);g.fillStyle='#19374b';g.font='bold 26px sans-serif';g.fillText(o.name+' · 적재 상세',50,55);g.font='20px sans-serif';const load=o.load;g.fillText('박스 '+load.boxWidth*1000+' × '+load.boxDepth*1000+' × '+load.boxHeight*1000+' mm · '+load.boxes.length+'개',50,95);g.fillText('팔레트 포함 총높이 '+Math.round(load.totalHeight*1000)+' mm · 평면 / 측면 (상세도 축척 별도)',50,130);const scale=Math.min(600/o.width,600/load.totalHeight),origin={x:70,y:190};g.strokeStyle='#335c74';g.fillStyle='#dbc19b';g.fillRect(origin.x,origin.y,o.width*scale,o.depth*scale);for(const b of load.boxes.filter(b=>Math.abs(b.z-load.palletHeight)<1e-6)){const f=footprint(b);g.beginPath();f.forEach((p,i)=>g[i?'lineTo':'moveTo'](origin.x+(p.x+o.width/2)*scale,origin.y+(p.y+o.depth/2)*scale));g.closePath();g.stroke();}const x=page.width/2+30,y=190+load.totalHeight*scale;g.strokeRect(x,y-load.palletHeight*scale,o.width*scale,load.palletHeight*scale);for(const b of load.boxes)g.strokeRect(x+(b.x+o.width/2-(b.rotation===90?b.depth:b.width)/2)*scale,y-(b.z+b.height)*scale,(b.rotation===90?b.depth:b.width)*scale,b.height*scale);pages.push(page);}
 return canvasesPDF(pages,mm);
}
function canvasesPDF(pages,mm){
 const enc=new TextEncoder(),chunks=[],offsets=[0];let size=0;const push=x=>{const b=typeof x==='string'?enc.encode(x):x;chunks.push(b);size+=b.length;};push('%PDF-1.4\n');const obj=(id,text)=>{offsets[id]=size;push(id+' 0 obj\n'+text+'\nendobj\n');},W=mm[0]*72/25.4,H=mm[1]*72/25.4;
 obj(1,'<< /Type /Catalog /Pages 2 0 R >>');obj(2,'<< /Type /Pages /Kids ['+pages.map((_,i)=>(3+i*3)+' 0 R').join(' ')+'] /Count '+pages.length+' >>');
 pages.forEach((c,i)=>{const id=3+i*3,jpeg=Uint8Array.from(atob(c.toDataURL('image/jpeg',.95).split(',')[1]),x=>x.charCodeAt(0));obj(id,'<< /Type /Page /Parent 2 0 R /MediaBox [0 0 '+W+' '+H+'] /Resources << /XObject << /Im0 '+(id+1)+' 0 R >> >> /Contents '+(id+2)+' 0 R >>');offsets[id+1]=size;push((id+1)+' 0 obj\n<< /Type /XObject /Subtype /Image /Width '+c.width+' /Height '+c.height+' /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length '+jpeg.length+' >>\nstream\n');push(jpeg);push('\nendstream\nendobj\n');const content='q '+W+' 0 0 '+H+' 0 0 cm /Im0 Do Q';obj(id+2,'<< /Length '+content.length+' >>\nstream\n'+content+'\nendstream');});const start=size;push('xref\n0 '+offsets.length+'\n0000000000 65535 f \n');for(let i=1;i<offsets.length;i++)push(String(offsets[i]).padStart(10,'0')+' 00000 n \n');push('trailer\n<< /Size '+offsets.length+' /Root 1 0 R >>\nstartxref\n'+start+'\n%%EOF');return new Blob(chunks,{type:'application/pdf'});
}
