...
export async function startFallback() {
  ...
  function sync() {
    ...
    [['바닥 면적', layout.warehouse.width * layout.warehouse.depth, '㎡'], ['배치 설비', layout.objects.length, '개'], ['팔레트 보관 위치', capacity(layout.objects).palletPositions, '위치'], ['경계 · 겹침', issues.length, '건']].forEach(([label, value, unit], i) => { $(`s${i + 1}l`).textContent = label; $(`s${i + 1}`).textContent = value; $(`s${i + 1}u`).textContent = unit; });
  ...
  function clearCredentials() { $('ghToken').value = ''; $('ghConsent').checked = false; }
  bind('githubBtn', () => $('githubDialog').showModal());
  bind('ghClose', () => { if (ghBusy) return; clearCredentials(); $('githubDialog').close(); });
  $('githubDialog').addEventListener('cancel', e => { if (ghBusy) e.preventDefault(); else clearCredentials(); });
  $('githubDialog').addEventListener('close', clearCredentials);
