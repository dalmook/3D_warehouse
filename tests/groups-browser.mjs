...
try{
 ...
 const l=normalize({projectName:'합성 묶음',warehouse:{width:60,depth:60,height:9},objects});await p.locator('#importFile').setInputFiles({name:'synthetic.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(l))});await p.locator('#topBtn').click();await p.locator('#multiAll').click();await p.locator('#scene').focus();await p.keyboard.press('Control+c');await p.keyboard.press('Control+v');await p.keyboard.press('r');const point=await p.evaluate(()=>window.__warehouseCity.screenPoint(30,30));await p.mouse.move(point.x,point.y);await p.keyboard.press('Enter');
