...
try{
 ...
 await p.locator('#routeOpen').click();await p.locator('#role-inbound').selectOption('in');await p.locator('#role-outbound').selectOption('out');await p.locator('#roleApply').click();await p.reload();await p.waitForFunction(()=>document.documentElement.dataset.ready==='true');assert.equal((await state()).routePoints[0].name,'검수 대기');
