...
try{
 ...
 for(const [type,def] of Object.entries(CATALOG)){
  ...
  await p.locator('#search').fill(def.name);await p.locator('#catalog .asset').filter({has:p.getByText(def.name,{exact:true})}).first().click();
  ...
  await p.reload();await p.waitForFunction(()=>document.documentElement.dataset.ready==='true');
