...
try{
 await p.goto(process.env.BASE_URL||'http://127.0.0.1:4173/');await p.waitForFunction(()=>document.documentElement.dataset.ready==='true');await dismissWelcome(p);await (await reveal(p,'blankBtn')).click();await (await reveal(p,'search')).fill('안내');
