...
export function installWorkbench(api) {
 ...
 const show = d => {for(const other of document.querySelectorAll('dialog[open]'))if(other!==d)other.close();d.showModal();};
 ...
 const focusObject=id=>{api.setMode('edit');api.cancelTool();api.select(id);api.focus?.(id);inspectorPanel('selection');$('right').classList.add('open');if(innerWidth<=850)$('left').classList.remove('open');$('right').scrollTop=0;};
 ...
 const scenarioKey='warehouse-city-workbench-scenarios-v1';let scenarioLoadError=false;
 try{const raw=JSON.parse(localStorage.getItem(scenarioKey)||'[]');if(!Array.isArray(raw))throw Error('invalid');storedScenarios=raw.map(s=>({...s,layout:normalize(s.layout)})).slice(0,5);}catch{scenarioLoadError=true;}
 const saveScenarios=next=>{localStorage.setItem(scenarioKey,JSON.stringify(next));storedScenarios=next;};
 ...
 const scenarioButton=button('wbScenarios','설계안 비교',()=>{renderScenarios();show(scenario);});project.prepend(scenarioButton);
 ...
 const openCommands=()=>{renderCommands();show(command);commandSearch.focus();};
 ...
 function onSelection(){const o=api.get().objects.find(o=>o.id===api.selected());selectionBar.hidden=!o||api.mode()!=='edit';selectionName.textContent=o?.name||'';for(const b of selectionBar.querySelectorAll('button'))b.disabled=!!o?.locked;if(o)inspectorPanel('selection');else inspectorPanel('overview');renderTree();}
 ...
 document.addEventListener('city-change',()=>{clearTimeout(refreshTimer);refreshTimer=setTimeout(()=>{refresh();onSelection();},80);});
