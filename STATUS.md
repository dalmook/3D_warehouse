...
검증 코드 커밋: `c190e42885ade3863cc8cd7e011f558545d06d9d`. 저사양 거리별 랙 LOD를 적용한 뒤 로컬 단위 30 및 소프트웨어 브라우저 22 검사를 다시 통과했다. 같은 도면의 저사양 조감도 34,358 triangles → 근접 300,280 triangles 복원을 확인했고 저장 데이터는 동일했다. 원격 실행 `35157871187`에서 단위·데스크톱/모바일·600랙·클래식·실제 GitHub 저장/수정/불러오기/토큰 비저장 단계가 모두 통과했다. 임시 브랜치 정리 후 matching-refs 조회 결과도 빈 배열이었다. 배포/공개 검증 job은 Pages 미설정으로 skipped다.

검수 갤러리: `http://127.0.0.1:4173/docs/evidence/index.html`. `tests/capture-operation.mjs`로 촬영한 15.8초 실제 운영·보행 WebM을 추가했다.
...
4. 권한·Pages 설정 문제 해결 후 최종 품질 승인, 보호 규칙에 따른 main 반영과 공개 URL 전체 재검증. c190e42의 실제 GitHub 저장 검증은 통과했으며 공개 사이트 원점 검증과는 구분한다.
