...
import {legacyModel,disposeLegacy} from './city-legacy-models.mjs';
...
export function thumbnail(type,definition){
 ...
 const model=['rack','boxrack'].includes(type)?rackAsset({...definition,type,config:{bays:2,levels:3,palletsPerLevel:1}}):fitAsset(type,definition)||legacyModel({...definition,type});
 ...
 const url=renderer.domElement.toDataURL();thumbnails.set(type,url);releaseInstance(model);disposeLegacy(scene);return url;
}
